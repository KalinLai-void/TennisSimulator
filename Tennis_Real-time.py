...
unityAddressPort = ("127.0.0.1", 65520)
...
# overhead
pX1, pY1 = 304, 103 # left-top corner
pX2, pY2 = 997, 103 # right-top corner
pX3, pY3 = 89, 693 # left-bottom corner
pX4, pY4 = 1211, 693 # right-bottom corner
# side
pX1s, pY1s = 274, 685 # left-top corner
pX2s, pY2s = 492, 507 # right-top corner
pX3s, pY3s = 1076, 685 # left-bottom corner
pX4s, pY4s = 841, 507 # right-bottom corner
...
# 1.7 is person's height (assume), 280 is person's pixel length (approximately)
...
EXAMINE_LINE_POINT_X = pX1s + int(math.dist((pX1s, 0), (pX2s, 0)) * 0.3) # for side-view
    ...
    if len(sidePointsAndTimestamps) > 1 and len(overheadPointsAndTimestamps) > 1: # has three balls frame
        ...
        if ((sidePoints[0][0] > sidePoints[1][0])
            ...
            if not isCalculating:
                ...
                startY = (sideStartPoint[1] - oY) * METERS_PER_PIXEL_Y
                startX = overheadStartPoint[1] * METERS_PER_PIXEL_X_FOR_OVERHEAD * COURT_W_SCALE
                startZ = overheadStartPoint[0] * METERS_PER_PIXEL_Z * COURT_H_SCALE
                ...
                sideProjDist = math.dist(sideEndPoint, sideStartPoint) * METERS_PER_PIXEL_X_FOR_SIDE * COURT_H_SCALE
                sideProjDist_X = sideProjDist * math.cos(math.radians(sideAngle))
                overheadProjDist = math.dist(overheadEndPoint, overheadStartPoint) * METERS_PER_PIXEL_X_FOR_OVERHEAD * COURT_H_SCALE
